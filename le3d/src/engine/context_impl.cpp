...
void glframeBufferResizeCallback(GLFWwindow* pWindow, s32 width, s32 height)
{
	if (pWindow == g_pWindow)
	{
		...
		inputImpl::callbacks().onResize(width, height);
	}
	return;
}
...
bool contextImpl::init(context::Settings const& settings)
{
	...
	if (glVersion < settings.ctxt.minVersion)
	{
		...
		LOG_E("FATAL: Incompatible OpenGL context: [%s]; required: [%s]", glVersion.toString(false).data(), v.toString(false).data());
		glfwTerminate();
		g_contextThreadID = {};
		return {};
	}
	...
	LOG_I("== [%s] OpenGL context created using %s [%s]", glVersion.toString(false).data(), renderer.data(), vendor.data());
	gfx::setMode(settings.ctxt.gfxMode);
	return true;
}
...
#endif
	gfx::enqueue([interval = g_context.swapInterval]() { glfwSwapInterval((s32)interval); });
	return;
}
...
void contextImpl::destroy()
{
	if (g_pWindow)
	{
		...
		inputImpl::clear();
		gfx::GFXStore::destroyInstance();
		gfx::setMode(GFXMode::ImmediateMainThread);
		glfwSetWindowShouldClose(g_pWindow, true);
